...
int main()
{
  ...
  char buf[1000];
  char bufrs[1000];               /*buffer of result of SEARCH*/
  char bufrp[1000];               /*buffer of result of PREFIX*/
  int server_sockfd;
  ...
  server_addr.sin_port=htons(PORT_SERVER_A);
  ...
  while(1)
  {
    ...
    if(recvfrom(server_sockfd,buf,sizeof(buf),0,(struct sockaddr*)&client_addr,&sin_size)<0) /*using UDP receive from awx*/ 
    {
    ...
    { 
       ...
       printf("Server A has found <%d> matches\n",manums);
       fclose(fp); 
